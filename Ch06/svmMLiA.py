...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            # 예제가 KKT(Karush-Kuhn-Tucker) 조건을 만족하는지 확인한다.
            """ KKT 조건
            등식의 제약식이 있는 최적화 문제를 푸는 것을
            '라그랑지 승수법'이라 하고,
            부등식의 제한 조건에서도 쓸 수 있게 확장시킨 것을
            'KKT 조건'이라 한다.
            """
...
def testRbf(k1=1.3):
    ...
    b, alphas = smoP(dataArr, labelArr, 200,
                     0.0001, 10000, ('rbf', k1))  # C=200 important
    datMat = mat(dataArr)
...
def innerLK(i, oS):
    ...
    if ((oS.labelMat[i] * Ei < -oS.tol) and (oS.alphas[i] < oS.C)) or ((oS.labelMat[i] * Ei > oS.tol) and (oS.alphas[i] > 0)):
        ...
        # oppostie direction
        updateEk(oS, i)
        b1 = oS.b - Ei - oS.labelMat[i] * (oS.alphas[i] - alphaIold) * oS.X[i, :] * oS.X[
            i, :].T - oS.labelMat[j] * (oS.alphas[j] - alphaJold) * oS.X[i, :] * oS.X[j, :].T
        b2 = oS.b - Ej - oS.labelMat[i] * (oS.alphas[i] - alphaIold) * oS.X[i, :] * oS.X[
            j, :].T - oS.labelMat[j] * (oS.alphas[j] - alphaJold) * oS.X[j, :] * oS.X[j, :].T
        if (0 < oS.alphas[i]) and (oS.C > oS.alphas[i]):
            oS.b = b1
        elif (0 < oS.alphas[j]) and (oS.C > oS.alphas[j]):
            oS.b = b2
